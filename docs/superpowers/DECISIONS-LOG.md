...
## 2026-06-21 — Deploy scripts were committed non-executable (Permission denied → whole deploy failed)
- **Context:** On the rebuilt box everything was correct (real service token, broker `http=200`, Access policy, tunnel config, DNS) but there were zero secrets/containers and Error 1033. Root cause: `fetch-secrets.sh` (and `deploy.sh`, `backup.sh`, `bootstrap-secrets.sh`) were committed mode **100644** — the Write tool creates files non-executable — so cloud-init's direct invocation `~/codevibes/deploy/fetch-secrets.sh` died with "Permission denied" (exit 126). No secrets → smoke fails → no containers → no connector → 1033. (`render-config.sh`/`gen-cloud-init.sh`/`apply.sh` were already 100755, which is why config.yml rendered but secrets didn't fetch.)
